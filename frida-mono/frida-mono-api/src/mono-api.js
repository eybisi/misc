...
var _fridaExNativefunction = _interopRequireDefault(
  require("frida-ex-nativefunction")
);
...
Object.keys(MonoApi).map((exportName) => {
  if (MonoApi[exportName] === null) {
    ...
    const addr = Module.findExportByName(_monoModule.default.name, exportName);
    MonoApi[exportName] = !addr
      ? () => {
          throw new Error("Export not found: " + exportName);
        }
      : (MonoApi[exportName] = new _fridaExNativefunction.default(
          addr,
          ...MonoApi[exportName]
        ));
  }

...
var Enumerator = {
  ...
  readString: function (addr) {
    ...
    var strStart = parseInt(addr) + 12; // when the 12th byte is 0x30, then the string starts at 16 bytes after the offset?
  ...
  // 4 bytes
  readQword: function (addr) {
    return this.readU64(addr);
  },
  // 8 bytes
  ...
  getClass: function (name) {
    ...
    _fridaMonoApi.MonoApiHelper.AssemblyForeach(function (assemb) {
      //send(assemb)
      var image = _fridaMonoApi.MonoApi.mono_assembly_get_image(assemb);
      
      var klass = _fridaMonoApi.MonoApiHelper.ClassFromName(image, name);
      if (klass != 0) {
        k = klass;
      }
    });
  ...
  getMethods: function (monoClass) {
    ...
    var methods = _fridaMonoApi.MonoApiHelper.ClassGetMethods(monoClass);

    for (var i = methods.length - 1; i >= 0; i--) {
      var method = methods[i];

      var name = _fridaMonoApi.MonoApiHelper.MethodGetName(method, monoClass);

      ret[name] = {
        address: '0x' + parseInt(method).toString(16),
        jit_address: '0x' + parseInt(_fridaMonoApi.MonoApi.mono_compile_method(method)).toString(16)
      };
    }
  ...
  getFieldType: function (field) {
    ...
    var monoValue = _fridaMonoApi.MonoApi.mono_type_get_type(_fridaMonoApi.MonoApi.mono_field_get_type(field));
  ...
  getFields: function (monoClass) {
    ...
    var fields = _fridaMonoApi.MonoApiHelper.ClassGetFields(monoClass);

    for (var i = fields.length - 1; i >= 0; i--) {
      var field = fields[i];

      var name = _fridaMonoApi.MonoApiHelper.FieldGetName(field, monoClass);
